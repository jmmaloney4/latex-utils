{ pkgs
  , lib
  , ...
}: with pkgs.lib.attrsets; with pkgs.lib; with pkgs.lib.debug; let
  ...
  in filesWithExtensions ++ recuriveFiles;
in
{ basePath, extensions ? [ ".tex" ".cls" ] }: processDirectory basePath extensions
